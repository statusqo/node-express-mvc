 ...
 */
async function sendOrderConfirmationEmail(order, lines = []) {
...
/**
 * Send event cancellation + refund notification. No-op if mail not configured.
 * @param {Object} opts - { to, eventTitle, startDate, startTime }
 */
async function sendEventCancellationEmail(opts = {}) {
  ...
  const text = `Your registration for "${eventTitle}" (${when}) has been cancelled. Your order has been refunded. If you have any questions, please contact us.`;
